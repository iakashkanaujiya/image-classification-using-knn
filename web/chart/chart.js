class Chart {
    ...
    showDynamicPoints(points, label, nearestSamples) {
    ...
    #getDataBounds() {
        const { samples } = this;
        const x = samples.map(s => s.points[0]);
        const y = samples.map(s => s.points[1]);
    ...
    #draw() {
        ...
        if (this.dynamicPoints) {
            ...
            graphics.drawPoint(ctx, pixelLoc, "rgba(255, 255, 255, 0.5)", 1000000);
    ...
    #drawSamples(samples) {
        ...
        for (const sample of samples) {
            const { points, label } = sample;
            const pixelLoc = math.remapPoint(
                dataBounds, pixelBounds, points
            );
            switch (this.icon) {
                case "image":
